...
class TestAnemometer(unittest.TestCase):
    ...
    def test_when_spinning_count_should_be_incremented_and_logged(self, mock_logging):
        ...
        mock_logging.debug.assert_called_once_with(msg=f'Signals count {self.test_anemometer.spin_count}.')
    ...
    def test_when_getting_sample_speed_expected_calls_should_be_done(self, mock_time, mock_logging):
        ...
        self.test_anemometer.calculate_speed.assert_called_once_with(current_spin_count=0, elapsed_seconds=test_finish_time - test_start_time)
