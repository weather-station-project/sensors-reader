...
APP_KEY = 'APP'
HEALTH_CHECK_CLASS = 'Health Check'
...
def register_error_in_health_check_file(key, message):
    ...
    logging.debug(msg=f'[{HEALTH_CHECK_CLASS}] Health check registered with key "{key}" and message "{message}".')
...
def register_success_for_class_into_health_check_file(class_name):
    ...
    logging.debug(msg=f'[{HEALTH_CHECK_CLASS}] Health check successful registered for class name "{class_name}".')
